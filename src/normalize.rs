...
pub fn decompose_compatible<F: FnMut(char)>(c: char, emit_char: F) {
    decompose(c, tables::compatibility_fully_decomposed, emit_char)
}

#[inline]
fn decompose<F, T>(c: char, table: T, mut emit_char: F)
    where F: FnMut(char), T: Fn(char) -> Option<&'static [char]>
{
    ...
    if let Some(decomposed) = table(c) {
        for &d in decomposed {
            emit_char(d);
        }
        return;
